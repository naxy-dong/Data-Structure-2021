...
/**
 * The PersonDataManager creates Person objects that are later stored in an
 * array.
 * It can manipute the data by features such as add, remove person from the
 * list.
...
public class PersonDataManager {
    ...
    /**
     * Uses the File class to read the .csv file and store the information in a data
     * structure
     * 
     ...
     *                                  format. For example, there is a letter in
     *                                  height or a number in the
     *                                  person’s name.
    ...
    public static void buildFromFile(String location) throws IllegalArgumentException {
        try (Scanner sc = new Scanner(new File(location));) {
            ...
            while (sc.hasNextLine()) {
                ...
                if (containsDigits(personData[0])
                        || !(personData[1].replaceAll("\"", "").equals("F")
                                || personData[1].replaceAll("\"", "").equals("M"))
                                || personData.length != 5) {
                    throw new IllegalArgumentException();
                }
                try {
    ...
    /**
     * Uses the File class to read the .csv file and return the number of lines in
     * the csv file
     * 
     * @param location File path of the file to be read as a String
     * @precondition valid file path
     ...
     * @return a boolean statement that indicates whether the str contains digits or
     *         not
     */
     ...
     *                                       biological statistics already exists in
     *                                       the list.
     */
    ...
    public void getPerson(String name) throws PersonDoesNotExistsException {
        ...
        for (Person person : people) {
            if (person.getName().equals(name)) {
                ...
                System.out.println(name + " is " + person.getAge() + " year old " + gender + " who is " + feet
                        + " feet and " + inches + " inches tall and weighs " + person.getWeight() + " pounds.");
                break;
            }
        }
        if (!found) {
    ...
    /**
     * Creates a file and stores all information from the current array into the
     * file
     * 
     * @param filename –
     * @precondition valid file path
    ...
    public void saveToFile(String filename) {
        try {
            ...
            for (Person person : people) {
                fWriter.write(person.getName() + "," + person.getGender() + "," + person.getAge() + ","
